...
| `extract` options | `--out file.md` saves a page; Markdown keeps content images; `--no-images` / `--no-links` give cleaner text. | Save articles or get text-only extracts. |
...
| `mcp` | Serve OmniScout to MCP clients (Claude Desktop, Cursor, opencode). | Connect any MCP client without a second install. |
...
| `screenshot` | Capture the screen to a file (`--region x,y,w,h` for part of the screen). | Visual debugging or logging. |
...
| `window` | List, activate, close, or resize desktop windows. | Focus a specific app before typing; resize any window with `--size`. |
...
---
## `computer window` sub‑commands (window management)
| Sub‑command | Purpose | Example |
|------------|---------|----------|
| `list` | List visible desktop windows. | `omniscout computer window list` |
| `activate` | Bring an app window to the front. | `omniscout computer window activate Terminal` |
| `close` | Close a window (requires `--confirm`). | `omniscout computer window close Terminal --confirm` |
| `resize` | Resize a window to any dimensions. | `omniscout computer window resize Terminal --size macbook_air` |

`resize` takes `--width`/`--height`, `--size WxH` (e.g. `--size 1280x800`), or a
device preset: `iphone-16-pro`, `iphone-16-pro-max`, `iphone-16`, `android`,
`ipad`, `ipad-pro-11`, `ipad-pro-12-9`, `macbook-air`, `macbook-pro-14`,
`macbook-pro-16`. Preset names are case-insensitive and accept spaces or
underscores (`--size "iPhone 16 Pro"`). Add `--title` to pick a window by title
hint; explicit `--width`/`--height` override preset dimensions.
...
7. **Take a screenshot**: `Omniscout computer screenshot` (or `--region 100,100,800,600` for part of the screen; browser pages also support `--ref "#hero"`, `--region x,y,w,h`, and `--stitched` for scroll-stitched full pages)
...
- Resize the browser viewport the same way: `omniscout browser resize --size iphone-16-pro` (or `--width`/`--height`, `--size WxH`).
